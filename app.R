...
# ==== packages (vanilla, prod-safe) ====
pkg_needed <- c("shiny","plotly","DT","readr","dplyr","tidyr","lubridate","scales","cachem","digest")

is_prod <- identical(Sys.getenv("SHINY_ENV"), "production")
...
# charge et échoue clairement si manquant
for (p in pkg_needed) {
  if (!requireNamespace(p, quietly = TRUE)) {
    stop(sprintf("Le package '%s' est requis. Installe-le avant de lancer l'app.", p))
  }
...
server <- function(input, output, session) {
  ...
  # Dark mode via JS + CSS (pas de bslib)
  observe({
